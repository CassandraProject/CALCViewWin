...
NAND performs a logical "nand" (not and) of the contents of the specified operands and stack 
locations-> Logical Not And is defined as producing a False output if all of the inputs are True 
and a True output if any one of the inputs is False.

If blank is specified, all stack locations are NANDed together, and the result is written into 
the accumulator (the new top of stack location). This overwrites the first operand and isolates 
the other operands from any further access-> sptr(after) = stackbottom.

If c is specified, the last c stack locations are NANDed together, removed from the stack, and the 
...
If only one operand is specified, the operand is NANDed with the accumulator, and the result 
replaces the accumulator. sptr(after) = sptr(before).

If both operands are specified, the operands are NANDed together and the result is stored in 
the accumulator. sptr(after) = sptr(before) + 1.
...
struct Instruction_NAND: public Instruction_Interface
{
    Instruction_NAND() {
    ...
    void Run (Steps_Table_Model &steps, Stack_Table_Model &stack, Machine &reg) {
        ...
        try {
            if (s->Empty_1()) {
                ...
                stack.Replace(result ? 0.0F : 1.0F);
