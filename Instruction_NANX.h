...
NANX reads all the values or a specified number (c) of values from the stack, performs a 16-bit 
bitwise logical "nand" function on them, and stores the result into the new accumulator location 
(the new top of stack). This overwrites the first operand and isolates the other operands from any 
further access-> Logical Not And is defined as producing a False output if all of the inputs are 
True and a True output if any one of the inputs is False.
...
struct Instruction_NANX: public Instruction_Interface
{
    Instruction_NANX() {
