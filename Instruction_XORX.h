...
XORX reads all the values or a specified number (c) of values from the stack, performs a 16-bit 
bitwise logical "xor" function on them, and stores the result into the new accumulator location 
(the new top of stack). This overwrites the first operand and isolates the other operands from 
any further access-> Exclusive Or is defined as producing a True output for an odd number of True 
inputs, a False output for an even number of True inputs, and a False output if all inputs are False.

For XORX, sptr(after) = stackbottom. For XORX c, sptr(after) = sptr(before) - c + 1.
...
struct Instruction_XORX: public Instruction_Interface
{
    Instruction_XORX() {
