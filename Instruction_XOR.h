...
XOR performs a logical "xor" of the contents of the specified operands and stack locations-> Exclusive 
Or is defined as producing a True output for an odd number of True inputs, a False output for an even 
number of True inputs, and a False output if all inputs are False.

If blank is specified, all stack locations are XORed together, and the result is written into the 
...
If c is specified, the last c stack locations are XORed together, removed from the stack, and the 
...
If only one operand is specified, the operand is XORed with the accumulator, and the result replaces 
...
struct Instruction_XOR: public Instruction_Interface
{
    Instruction_XOR() {
