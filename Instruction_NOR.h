...
NOR performs a logical "nor" (not or) of the contents of the specified operands and stack locations-> 
...
If blank is specified, all stack locations are NORed together, and the result is written into the 
...
If c is specified, the last c stack locations are NORed together, removed from the stack, and the 
...
If only one operand is specified, the operand is NORed with the accumulator, and the result replaces 
...
struct Instruction_NOR: public Instruction_Interface
{
    Instruction_NOR() {
