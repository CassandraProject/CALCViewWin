...
SST sets the specified operand to 1.0 (if ROxx or Mxx) or 1 (if IOxx or LOxx) or True (if BOxx) 
and skips the next program step when the accumulator is non-zero. Otherwise the instruction is 
...
is non-zero, the operand is set and then the block program terminates->
...
struct Instruction_SST: public Instruction_Interface
{
    Instruction_SST() {
