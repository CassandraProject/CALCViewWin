...
AND performs a logical "and" of the contents of the specified operands and stack locations->

If blank is specified, all stack locations are ANDed together, and the result is written 
into the accumulator (the new top of stack location). This overwrites the first operand and 
isolates the other operands from any further access-> sptr(after) = stackbottom.

If c is specified, the last c stack locations are ANDed together, removed from the stack, and 
the result stored in the accumulator. sptr(after) = sptr(before) - c + 1.

If c exceeds the current number of stack operands, a "6" (stack underflow run-time error) is 
written to the PERROR parameter.

If only one operand is specified, the operand is ANDed with the accumulator, and the result 
...
struct Instruction_AND: public Instruction_Interface
{
    Instruction_AND() {
