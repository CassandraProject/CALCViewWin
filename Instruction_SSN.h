...
SSN sets the specified operand to 1.0 (if ROxx or Mxx) or 1 (if IOxx or LOxx) or True (if BOxx) and 
skips the next program step when the accumulator is negative. Otherwise the instruction is ignored 
...
is negative, the operand is set and then the block program terminates->
...
struct Instruction_SSN: public Instruction_Interface
{
    Instruction_SSN() {
