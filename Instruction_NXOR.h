...
NXOR performs a logical "nxor" of the contents of the specified operands and stack locations-> Not 
Exclusive Or is defined as producing a True output for an even number of True inputs, a False output 
for an odd number of True inputs, and a True output if all inputs are False.
...
If c is specified, the last c stack locations are NXORed together, removed from the stack, and the 
...
If only one operand is specified, the operand is NXORed with the accumulator, and the result replaces 
...
struct Instruction_NXOR: public Instruction_Interface
{
    Instruction_NXOR() {
