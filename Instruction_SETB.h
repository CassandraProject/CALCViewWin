...
SETB sets to 1 a specified bit in the accumulator. If blank is specified the bit number is expected to be at the top of the stack and this bit number is removed from the stack before the instruction is executed. sptr(after) = sptr(before) - 1.

If the bit number is less than 1 or greater than 16 an "11" (bit error run-time error) is written to the PERROR parameter.

If the operand b is specified it indicates a bit number between 1 and 16 in the accumulator. The stack is not popped in this case. sptr(after) = sptr(before).
...
struct Instruction_SETB: public Instruction_Interface
{
    Instruction_SETB() {
