...
struct Instruction_LN: public Instruction_Interface
{
    Instruction_LN() {
    ...
    void Run (Steps_Table_Model &steps, Stack_Table_Model &stack, Machine &reg) {
        ...
        try {
            ...
            if (acc <= 0.0F) {
                throw Exception_LN_Error();	// error code
            } else {
                acc = (float) std::log(acc);
                stack.Replace(acc);
            }
        }
	    ...
	    steps.Increment_Step();
            if (reg.Index_To_Memory(PERROR)->Get_Value() == 0.0F)
                reg.Index_To_Memory(PERROR)->Set_Value(8);	// error code
            throw e;
        }
		steps.Increment_Step();
	}
